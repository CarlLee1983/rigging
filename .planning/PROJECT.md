...
- [ ] **CI-01**: CI 首跑綠燈（push + PR → GitHub Actions 3 jobs all green）
- [ ] **CI-02**: CI fail-mode 可觀測（驗證 lint/typecheck/test/drift 任一破壞時 gate 會擋）
- [ ] **SEC-01**: Phase 04 SECURITY audit（`$gsd-secure-phase 04` retroactive threat-mitigation audit）
- [ ] **ADR-01**: ADR 機制 self-check（adr-check workflow 擋格式錯誤驗證 + status 欄整理）
