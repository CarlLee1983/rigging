...
## Current Milestone: v1.2 Create Rigging

**Goal:** 把 reference app 轉為可分發的 scaffold CLI，讓開發者 `npx create-rigging my-app` 拿到完整的 harness 起點並自動完成名稱替換，發布至 npm。

**Target features:**
- `create-rigging` npm 套件（CLI 入口）
- `npx create-rigging <project-name>` 生成完整專案（DDD 四層 + AuthContext + demo domain + tests + CI）
- 自動名稱替換（package.json、import 路徑等）
- 生成的專案開箱即過 `bun test`
- npm 公開發布（任何人可 npx 執行）
- 使用者文件更新（README + docs/quickstart.md）
...
<!-- v1.2 — populated via $gsd-new-milestone 2026-04-20 -->

- [ ] **SCAF-01**: Developer can run `npx create-rigging <name>` and get a fully working project directory
- [ ] **SCAF-02**: Generated project's package name and relevant identifiers are automatically substituted with the given project name
- [ ] **SCAF-03**: Generated project passes `bun test` out of the box (all tests green)
- [ ] **SCAF-04**: Generated project includes ready-to-use GitHub Actions CI workflow
- [ ] **SCAF-05**: `create-rigging` package is published to npm (public, `npx` invocable)
- [ ] **SCAF-06**: `.planning/` and scaffold-internal files are excluded from the generated project
- [ ] **SCAF-07**: Usage documentation updated (README + `docs/quickstart.md`) to cover scaffold installation

**Deferred to v1.3+（from v1.0/v1.1/v1.2 候選池，待下個 milestone 重議）：**
...
---
*Last updated: 2026-04-20 — v1.2 Create Rigging milestone started*
