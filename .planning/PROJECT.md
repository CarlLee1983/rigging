...
**Active milestone:** v1.1 Release Validation（2026-04-20 啟動）— 收尾 milestone，聚焦 CI 首跑綠燈 / Phase 04 SECURITY audit 補票 / ADR 機制 self-check / Observability smoke in CI，讓 v1.0 shipped 從「理論 complete」轉為「操作 self-verified」。scaffold extraction / production hardening / extended identity 延後至 v1.2+。

**v1.1 progress (2026-04-20):** Phase 6 complete — CI-04 + CI-05 + OBS-01 all validated。PR #1 merged（首綠 baseline + smoke step），PR #2 closed not merged（5-row fail-mode matrix）。Next: Phase 7 (SEC-01) / Phase 8 (ADR-06)。
...
## Current Milestone: v1.1 Release Validation

**Goal:** 關閉 v1.0 遺留的收尾項目（CI 首跑綠燈、Phase 04 SECURITY audit、ADR 機制 self-check、Observability smoke），讓 Rigging 在進入 scaffold 拆分或 production hardening 前站穩 release-ready 的起跑點。

**Target features:**

- **CI 首跑綠燈驗證** — push branch + 開 PR → GitHub Actions 3 parallel jobs（lint / typecheck / test+coverage / migration drift）首次實跑綠燈；必要時補 fail-mode 驗證讓 gate 行為可觀測
- **Phase 04 SECURITY audit 補票** — 執行 `$gsd-secure-phase 04` 對 Phase 04 ship 的 auth-gated API + API Key hash verify 做 threat-mitigation retroactive audit；產出 `phases/04-demo-domain/SECURITY.md`
- **ADR 機制 self-check** — 本 milestone 開 PR 時驗證 `adr-check` workflow 能擋格式錯誤；視情況補 ADR 0019（e.g. CI pipeline 決策）或整理 status 欄
- **Observability smoke in CI** — 新增 `createApp` boot + `/health` real HTTP ping 的輕量 smoke step，避免「型別過 / 測試過 / 但實際 startup 炸」gap

**Key context:**
- 這是 **hygiene / 收尾 milestone**——不新增 product feature，專心驗證 v1.0 shipped 狀態 self-verifiable
- 完成後才適合啟動 v1.2+（scaffold extraction 或 production hardening 或 extended identity）
- 技術棧鎖定不變（Bun + Elysia + DDD），預期零新 runtime dependency
- Phase 編號從 **Phase 6 開始**（v1.0 結束於 Phase 5，本 milestone 未使用 `--reset-phase-numbers`）
...
<!-- v1.1 Release Validation — REQ-IDs finalised in .planning/REQUIREMENTS.md -->

- [ ] **SEC-01**: Phase 04 SECURITY audit（`$gsd-secure-phase 04` retroactive threat-mitigation audit）
- [ ] **ADR-06**: ADR 機制 self-check（adr-check workflow 擋格式錯誤驗證 + status 欄整理）

**Deferred to v1.2+（from v1.0 候選池，待 v1.1 後重議）：**
...
---
*Last updated: 2026-04-20 — Phase 6 complete (CI-04 + CI-05 + OBS-01 validated); v1.1 progress 1/3 phases*
