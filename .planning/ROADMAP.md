...
**Plans**: TBD (estimate: 5 plans — BetterAuth schema gen + Drizzle migration + spike、auth domain + ports、auth infrastructure（repos + adapters + BunPasswordHasher + ConsoleEmailAdapter）、use cases（register / verify / reset / API Key CRUD）、authContext plugin + macro + resolver + Runtime Guards + regression tests)
**UI hint**: no
