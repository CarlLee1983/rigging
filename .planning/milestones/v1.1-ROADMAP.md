...
**Phases:** 6 → 8
**Total Plans:** 5 plans (2 + 1 + 2)
**Timeline:** 2026-04-20 (execution)

## Overview

v1.1 關閉 v1.0 遺留的 release hygiene：在真實 GitHub PR 上驗證 CI 全綠與 fail-mode、補齊 Phase 04 的 retroactive SECURITY 證據、並證明 ADR 流程（MADR + `adr-check` + README 索引）在 PR 實務可擋錯誤。不新增 product feature，專注 self-verified operations。
...
- ADR 0019 記錄 CI smoke + `validate:adr` / `adr-check` 整合（見 `docs/decisions/0019-adr-check-madr-validation.md`）
- Fail-mode 驗證採「sacrificial PR closed not merged」保留證據而不污染 main

**Issues Resolved:**

- v1.0 Known Gaps 中「CI 首跑」「Phase 04 SECURITY audit」「ADR self-check」於本 milestone 關閉

**Issues Deferred:**

- Phase 3 `03-VERIFICATION.md` 仍標 `human_needed`（端到端 stdout 驗證連結）；自動化分數 7/7，見 close audit acknowledgment

**Technical Debt:**

- Integration tests 共用 Postgres（ADR 0018）— 仍列為 ⚠️ Revisit（與 v1.0 相同）
