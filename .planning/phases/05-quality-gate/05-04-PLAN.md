...
- Do NOT add backreferences to v2 PROD-* by ADR ID (those don't exist yet) — only mention them prosaically as "v2 PROD-* may revisit"
  ...
  <acceptance_criteria>
    ...
    - All 18 ADR index rows present (0000-0018, matches both `| [0000]` and `| 0000` formats): `grep -cE "^\| \[?00(0[0-9]\|1[0-8])" docs/decisions/README.md | awk '{ exit ($1 != 18) }'`
    - Status casing consistent (all 18 rows use `accepted` lowercase per existing convention): `grep -cE "\| accepted \|" docs/decisions/README.md | awk '{ exit ($1 != 18) }'`
    - `bun run lint` exits 0
  </acceptance_criteria>
