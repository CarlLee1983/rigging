...
1. Add a new row for ADR 0018 in numeric order (after 0017):
   ...
   ```
2. Audit all existing rows (0000-0017): if any Status column says `Draft`, `TBD`, `Proposed` (for ADRs whose decisions have been shipped in P1-P4), update to `Accepted`. Verify by reading each ADR file's frontmatter — the source-of-truth status is in the ADR file, the index reflects it.
3. If existing index has dates that read as the date the ADR was first drafted vs accepted, leave as-is — only change Status column.
...
- Do NOT add backreferences to v2 PROD-* by ADR ID (those don't exist yet) — only mention them prosaically as "v2 PROD-* may revisit"
  ...
  <acceptance_criteria>
    ...
    - No placeholder Status for committed ADRs 0000-0017: `! grep -E "^\| 00(0[0-9]\|1[0-7]).*\| (TBD\|Draft) " docs/decisions/README.md` exits 0 — **G20**
    - All 18 ADRs (0000-0018) listed: `grep -cE "^\| 00(0[0-9]\|1[0-8])" docs/decisions/README.md | awk '{ exit ($1 != 18) }'`
    - `bun run lint` exits 0
  </acceptance_criteria>
...
- TOC bullets MUST use English-anchor where possible for cross-doc safety; the Rigidity Map bullet (CJK) is unavoidable since the heading itself contains CJK — but the HTML `<a id>` at the file top is the cross-doc safe entry that README.md uses
  ...
  <acceptance_criteria>
    ...
    - GSD-managed blocks preserved: `grep -c "^<!-- GSD:" AGENTS.md` returns same count as before edit (planner records baseline in 05-04-SUMMARY)
